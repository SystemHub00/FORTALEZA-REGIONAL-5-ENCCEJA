...
<head>
    ...
    fbq('init', '1040595512174509');
    fbq('track', 'PageView');
    </script>
    <noscript><img height="1" width="1" style="display:none"
    src="https://www.facebook.com/tr?id=1040595512174509&ev=PageView&noscript=1"
    /></noscript>
    <!-- End Meta Pixel Code -->
    <style>
...
<body data-start-step="{{ current_step }}">
                    ...
                    <div class="step-card">
                        ...
                        <div class="review-layout">
                            ...
                            <div class="review-box full">
                                ...
                                <div style="margin-top:10px;"><strong>Ao confirmar voc&#234; declara a ci&#234;ncia de que:</strong><ul><li>O curso &#233; totalmente gratuito</li></ul></div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            var courseSelect=document.getElementById('curso_id'),localSelectEl=document.getElementById('local_id_select'),opcaoSelectEl=document.getElementById('opcao_id_select'),localGroup=document.getElementById('local-group'),turmaGroup=document.getElementById('turma-group');
            ...
            function atualizarLocaisPorCurso(cursoId,selectedLocalId,selectedOpcaoId){var turmasDoCurso=courseOptions.filter(function(o){return String(o.curso_id)===String(cursoId||'');});var localIdsUsados=[],locaisUnicos=[];turmasDoCurso.forEach(function(o){if(localIdsUsados.indexOf(o.local_id)===-1){localIdsUsados.push(o.local_id);var lObj=localOptions.find(function(l){return String(l.id)===String(o.local_id);});if(lObj)locaisUnicos.push(lObj);}});localSelectEl.innerHTML='';opcaoSelectEl.innerHTML='';turmaGroup.style.display='none';aplicarOpcao('');if(!locaisUnicos.length){localGroup.style.display='none';return;}localGroup.style.display='';var ph=document.createElement('option');ph.value='';ph.textContent='Selecione um local';localSelectEl.appendChild(ph);locaisUnicos.forEach(function(loc){var opt=document.createElement('option');opt.value=loc.id;opt.textContent=loc.nome;if(String(loc.id)===String(selectedLocalId||''))opt.selected=true;localSelectEl.appendChild(opt);});if(locaisUnicos.length===1&&!selectedLocalId){localSelectEl.value=locaisUnicos[0].id;atualizarHorariosPorLocal(cursoId,locaisUnicos[0].id,selectedOpcaoId);return;}if(selectedLocalId&&localIdsUsados.indexOf(String(selectedLocalId))!==-1){atualizarHorariosPorLocal(cursoId,selectedLocalId,selectedOpcaoId);}}
            function atualizarHorariosPorLocal(cursoId,localId,selectedOpcaoId){var turmasFiltradas=courseOptions.filter(function(o){return String(o.curso_id)===String(cursoId||'')&&String(o.local_id)===String(localId||'');});opcaoSelectEl.innerHTML='';if(!turmasFiltradas.length){turmaGroup.style.display='none';aplicarOpcao('');return;}turmaGroup.style.display='';var ph=document.createElement('option');ph.value='';ph.textContent='Selecione um hor\u00e1rio';opcaoSelectEl.appendChild(ph);turmasFiltradas.forEach(function(op){var opt=document.createElement('option');opt.value=op.id;opt.textContent=op.dias_aula+' | '+op.horario;if(String(op.id)===String(selectedOpcaoId||''))opt.selected=true;opcaoSelectEl.appendChild(opt);});if(selectedOpcaoId&&turmasFiltradas.some(function(o){return String(o.id)===String(selectedOpcaoId);})){aplicarOpcao(selectedOpcaoId);}else if(turmasFiltradas.length===1){opcaoSelectEl.value=turmasFiltradas[0].id;aplicarOpcao(turmasFiltradas[0].id);}else{aplicarOpcao('');}}
...
<head>
    ...
    <style>
        ...
        .logo-prefeitura-topo{height:52px;width:auto;object-fit:contain;}.logo-projeto-topo{height:52px;width:auto;object-fit:contain;}
        ...
        .checkmark{width:120px;height:120px;margin:0 auto 12px;}.checkmark svg{width:100%;height:100%;stroke:#8b0000;fill:none;}
...
def inscricao_unica():
    ...
    try:
        ...
        print("Erro ao salvar na planilha:", exc); traceback.print_exc()
    try:
